...
class HeartDiseasePredictor:
    ...
    def predict(self, features_df):
        ...
        try:
            ...
            inverted_prediction = 1 - original_prediction[0]
            inverted_probabilities = [
                1 - original_prediction_proba[0][1],
                original_prediction_proba[0][1],
            ]

            print("\nInverted Prediction Results:")
            print(
                f"Prediction: {'High Risk' if inverted_prediction == 1 else 'Low Risk'}"
            )
    ...
    with st.container():
        ...
        with col1:
            trestbps = st.number_input(
                "Resting Blood Pressure", min_value=90, max_value=200, value=120
            )
        with col2:
            chol = st.number_input(
                "Serum Cholesterol", min_value=100, max_value=600, value=200
            )
        with col3:
        ...
        with col1:
            oldpeak = st.number_input(
                "ST Depression", min_value=0.0, max_value=6.2, value=0.0
            )
        with col2:
            slope = st.selectbox(
        ...
        with col3:
            ca = st.number_input(
                "Number of Major Vessels", min_value=0, max_value=3, value=0
            )
        ...
        with col1:
            ...
            thal_values = {"Normal": 0, "Fixed Defect": 1, "Reversible Defect": 2}
...
def show_heart_health_advisor(prediction):
    ...
    with st.expander("Click for Personalized Recommendations"):
        if prediction == 1:
            ...
            st.success(
                "Great news! Maintain your heart health with these recommendations:"
            )
            st.markdown(
                """
